/**
 * File is auto-generated"
 * !!! Avoid any changes to this file !!!"
 * In case you need to generate new file run `node bin/generators/generators.index` from project root",
 */

import * as $_ToLowerCase from "./transformers.to-lower-case";
import * as $_ToSnakeCase from "./transformers.to-snake-case";
import * as $_ToUpperCase from "./transformers.to-upper-case";

export namespace Transformers {
    export import ToLowerCase = $_ToLowerCase._ToLowerCase;
    export import ToSnakeCase = $_ToSnakeCase._ToSnakeCase;
    export import ToUpperCase = $_ToUpperCase._ToUpperCase;
}
