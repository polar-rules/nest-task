...
describe("Core::ProjectConfiguration::Task", (): void => {
    const Subject = Core.ProjectConfiguration.Task;
    ...
    describe("#path", (): void => {
        it("Should correctly resolve path", (): void => {
            ...
            expect(
                subject.path.endsWith(path.join(Configs.Constants.Folders.tmp, entryPoint, "example.task.ts")),
            ).toBeTruthy();
        });
